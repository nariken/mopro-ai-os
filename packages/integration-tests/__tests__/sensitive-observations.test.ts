...
describe("sensitive observations", () => {
  ...
  it.concurrent(
    ...
    await withSession(async publicApi => {
      ...
      // residual coverage a later re-grant would trust unverified. That cleanup needs the
      // verification to commit through a gate run synchronously with the record persist; see the
      // commit-gate TODO in authorizeCollaborator, fixed in PR #306 commit 0f39a797.)
      await expect(ws.overseer.listCollaborators()).resolves.toEqual([]);
    });
  });
