...
- **Role-based breadth of verification:**
  ...
  - **`use`** collaborators (UI only, no chat access — see `UseOverseerInterface` in
    `overseer.ts`) must be verified only against **named bindings** (gatekeepers with a
...
less verification than `open()` would demand. (Known limitation, see PR #306: the external path's
entry check is separated from the chat commit by real await windows, and a caller whose coverage
was scrubbed or role severed in that window still commits; re-asserting the gate synchronously
with every write the submission justifies is #306's `83c9f18e`.)
...
5. **Re-verify all in-scope bindings** (covered + newly chosen). For each, resolve the chosen
   ...
   - If any `addObserver` **throws** (or `getVerifier` throws on vendor mismatch, or returns null
     ...
     *persisted* record, so the coverage guard (edge case 4) fails closed immediately, and the
     ...
     record is persisted. (Known limitation, see PR #306: for a *re-verification* of an admitted
     observer that rollback is too broad — the registration is what preserves forward exclusion
     for their still-live sessions, so re-asserted registrations should be kept; the first-ever
     vs. re-verification split, and the cleanup when a racing teardown deleted the record
     mid-call, are #306's `546a9643`/`4db373a1`.)
...
1. Map the opaque `observerId` → `profileId` via the `observers.byObserverId` index. If there is
   no record, the id is not an active observer → ignore it. (Known limitation, see PR #306: a
   first-time verification registers its freshly minted id with gatekeepers before the record is
   persisted, and that window spans awaits — sibling verifier RPCs, even the configuration modal
   — so an observation naming such an id is read as unknown and admitted, moments before the
   collaborator is admitted with the data already in chat history. Tracking mid-registration ids
   in an in-memory pending map and failing closed on them is #306's `7ba93821`.)
...
(Known limitation, see PR #306: the DO abort that actually ends a removed user's live sessions
runs only after the awaited teardown and listing-refresh phases, and once the teardown's
`removeObserver` fan-out de-registers the observer, gatekeepers stop naming them in
`excludeObservers` at all — so no per-gate check can see the removed user across that window (the
coverage guard's zero-collaborators early return is likewise blind when the *last* collaborator
was removed). Failing **every** observation closed until the revocation restart lands is #306's
`a0938b79`/`aa6d6987`. See `OverseerImpl.scheduleRevocationRestart`.)
...
- After a mutation, use the returned `AffectedCollaborator[]` to find users who **lost access**.
  ...
  then best-effort `removeObserver(record.observerId)` on **all** gatekeeper facets. (Known
  limitation, see PR #306: the awaited fan-out here is part of why the revocation restart is not
  immediate, and observations arriving before it lands are not failed closed — #306's
  `a0938b79`/`aa6d6987`.)
...
- All these calls are best-effort: log and continue on error. An orphaned observer entry only
  ...
  persisted for a party who passed full verification. (Known limitation, see PR #306: a share-key
  redemption denied *after* verification succeeded — topology change, revoked link — currently
  leaves the record behind, because the denial runs after the record write; running it as a
  commit gate before the write is #306's `7fd354f4`/`0f39a797`.)
...
8. **Removing a connection that read restricted data** — while the workspace is latched
   (`containsRestrictedData`) *and* shared, `GatekeeperClient.remove()` refuses: the gatekeeper
   record is what observer verification (and the coverage guard) runs against, and the restricted
   data outlives the connection in chat history and storage, so deleting the record would let a
   never-verified collaborator open unchecked. Outstanding (unrevoked) share links block removal
   the same way: a link creates no collaborator state until redeemed, keys are multi-redeemable
   and never expire, and redemption is gated at open() only while the record exists — so a link
   redeemed after removal would grant the same unchecked access. The remedy is to remove
   collaborators and revoke share links first.
   ...
   *pending* write (`redeemShareKey`), and again in the *confirm's* synchronous block
   ...
   refuses the grant. (Known limitation, see PR #306: the confirm-side refusal does not yet roll
   back the observer record `ensureObserver` already persisted — running the confirm as a commit
   gate immediately before the record write is #306's `7fd354f4`.) Existing grants are untouched: a *confirmed* edge skips both checks, so a
