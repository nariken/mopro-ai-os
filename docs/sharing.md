...
The abort also lands later than the ~100ms delay alone suggests: the revocation handlers first await the observer teardown (`tearDownLostObservers`, a per-collaborator `removeObserver` fan-out) and the listing refresh (`refreshAffectedCollaboratorListings`, chunked cross-DO round trips), so the removed users' sessions stay live and watching for a window that scales with collaborator and gatekeeper count. (Known limitation, see PR #306: observations arriving in that window are admitted -- every per-observation check reads the removed user as gone, since their observer record is deleted at the sever, removing the last collaborator empties the coverage guard's list, and once the teardown's `removeObserver` fan-out completes, gatekeepers stop naming them in `excludeObservers` at all. Failing every observation closed across the window is #306's `a0938b79`/`aa6d6987`.)
...
One party the restart deliberately does not cover is a redeemer whose share link is revoked *while their first open is still verifying them*: their edge is pending, so it is invisible to the revocation's affected-set computation and no restart targets them. They are denied anyway -- after verification, `authorizeCollaborator` re-derives the role from the live graph, where the revoked link contributes nothing. The same re-derivation runs for keyless opens too: a collaborator removed while their re-verification was parked is denied by it rather than admitted at their stale role, and a downgrade landing in the same window caps the returned capability at the live role. (Known limitation, see PR #306: this re-check runs after verification's final record persist, so it cannot stop that persist from resurrecting an observer record the removal's teardown just deleted; re-checking the role synchronously with the persist -- the commit gate -- is #306's `334e2eb5`/`428d09bd`.)
